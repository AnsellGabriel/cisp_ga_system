class VotesController < ApplicationController
  ...
  def vote
    ...
    if params[:p].present?
      @elect_position = @coop_event.elect_positions.find(params[:p])
    else
      @elect_position = @coop_event.elect_positions.first
    end
    @next = @elect_position.id + 1
    @next_position = @coop_event.elect_positions.find_by(id: @next)
