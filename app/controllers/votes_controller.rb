class VotesController < ApplicationController
  ...
  def vote
    @event_hub = EventHub.find(params[:e])
    if params[:p].present?
      @elect_position = ElectPosition.find_by(id: params[:p])
    else
      @elect_position = @my_coop_event.elect_positions.first
    end
    @next = @elect_position.id + 1
    ...
    @candidates = Candidate.where(coop_event_id: @my_coop_event.id, elect_position: @elect_position)
    # raise "error"
  ...
  def result
    @elect_position = @my_coop_event.elect_positions
